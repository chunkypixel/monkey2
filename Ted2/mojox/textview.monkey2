...
'-- MS --
	Field _cursorLast:Int
	Field _clickCount:Int
	...
	Method ExpandSelect(p:Vec2i)
		...
		Local clickLine:=p.y/_charh
		If (clickLine>_doc.LineCount) Return
		
		Local length:Int=Max(0,_doc.LineLength(clickLine)-_doc.LineLength(clickLine-1)-1)
		If (length<1) Return
		
		Local cursorCurrent:Int=_cursor		
		If (_cursorLast=_cursor)
			p.x=0
			...
			Local chr:String=text.Mid(startPos, 1).ToLower()
			If (alpha.Find(chr)=-1)
				'Mark line?
				If (_clickCount>0)
					_anchor=cursor
				...
				Repeat
					startPos-=1
					chr=text.Mid(startPos, 1).ToLower()
					pos+=1
				Until (startPos<0 Or alpha.Find(chr)>-1)
				_anchor-=pos
				...
				Repeat
					endPos+=1
					chr=text.Mid(endPos,1).ToLower()
					pos+=1
				Until (endPos>=length Or alpha.Find(chr)>-1)
				_cursor+=pos
			...
			Else
            	...
            	'Mark line?
				If (mark Or (_anchor=_clickedStart And _cursor=_clickedEnd))
					_anchor=cursor
					_cursor=cursor+length
					Return
				End
