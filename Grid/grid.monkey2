...
Public
	...
	Method New()
		'Create window
		Super.New( "Grid!",640,480,WindowFlags.Resizable )
		Layout="letterbox"
		...
		'Initialise grid
		_grid=New GridManager(Size.X,Size.Y)
	...
	Method OnRender( canvas:Canvas ) Override
		'Features
		If (Keyboard.KeyHit(Key.G)) _grid.Style=(_grid.Style + 1) Mod 10
		
		'Animate grid?
		If(Rnd(0,100)>90)
			Local animation:Float=Rnd(0,2) 
			If (animation<=1.50)
				_grid.Shockwave(Rnd(0,Size.X),Rnd(0,Size.Y))
			Elseif (animation>1.50 And animation<=1.70)
				_grid.Pull(Rnd(0,Size.X),Rnd(0,Size.Y),64,10)
			Elseif (animation>1.70 And animation<=1.90)
				_grid.BombShockwave(Rnd(0,Size.X),Rnd(0,Size.Y))
			Else
				_grid.Push(Rnd(0,Size.X),Rnd(0,Size.Y),64,2.5)
			End
		End
		...
		canvas.DrawText("Style: "+_grid.Style,4,App.ActiveWindow.Height-40)
		canvas.DrawText("Press 'G' to change",4,App.ActiveWindow.Height-25)
